...
namespace WebApplication1
{
    ...
    public partial class Reservation_Ticket_Selection : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            /****ここからロードイベントの本体****/
            /************************************/
            int backflag = 0;//前の画面から戻るボタンで戻ったときに1を格納する
            if ((string)Session["PageID"] == "Reservation_Confirm_Input_Information_Back" && !IsPostBack)
                backflag = 1;
            else if ((string)Session["PageID"] != "Screen_Next" && !IsPostBack)//正規の手順でこのフォームを表示しなかった場合
            {
                //各Sessionのクリア
                ...
                Response.Redirect("Screen01.aspx");//TOP画面に飛ぶ
                return;
            }
            ...
            if (scheduleflag == 0)//スケジュール番号が登録されていなければ
            {
                ...
                {
                    if (backflag == 0 || IsPostBack)
                    {
                        OleDbDataAdapter daMember = new OleDbDataAdapter("SELECT MEMBER_NAME,MEMBER_POINT,MEMBER_MAIL FROM TBL_MEMBER WHERE MEMBER_ID='" + (string)Session["MemberID"] + "'", cn);
                        ...
                        Session["BookingMail"] = dtMember.Rows[0][2].ToString();
                        //ポイントテーブル
                    ...
                    displayeventnumber[ticketcount] = "000";
                    displaydefaultnumber[ticketcount] = "000";
                    displayticketname[ticketcount] = Session["PointUse"].ToString() + "ポイントで一回無料";
        ...
        {
            Session["PageID"] = "Reservation_Ticket_Selection_Back";
            //仮予約テーブルの該当する席を削除する
            OleDbConnection cn = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;" + "Data Source=|DataDirectory|BookingDB.accdb;");
            ...
            Response.Redirect("Screen" + Session["Screen"].ToString() + ".aspx");
        }
        ...
        protected void NextBtn_Click(object sender, EventArgs e)
        {
            ...
            Session["PageID"] = "Reservation_Ticket_Selection_Next";
            Response.Redirect("Reservation_Confirm_Input_Information.aspx");
            //ClientScriptManager cs = Page.ClientScript;
            //Type csType = this.GetType();
            //cs.RegisterStartupScript(csType, "onclick", "<script type=\"text/javascript\">location.replace('https://www.google.co.jp');</script>");
        }
